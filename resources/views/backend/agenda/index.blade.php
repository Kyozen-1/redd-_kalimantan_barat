...
@section('title', 'Agenda | REDD++ Kalimantan Barat')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_agenda" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th>Aksi</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_agenda" method="POST" data-parsley-validate novalidate>
                        @csrf
                        <div class="form-group">
                            <label for="nama" class="control-label">Nama Agenda<span class="text-danger">*</span></label>
                            <input type="text" name="nama" id="nama" parsley-trigger="change" required
                            placeholder="Masukan nama agenda..." class="form-control">
                        </div>
                        <div class="form-group">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="detail_nama" class="control-label col-md-6">Nama Agenda</label>
                        <div class="col-md-6">
                            <span id="detail_nama"></span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="detail_tanggal" class="control-label col-md-6">Tanggal Agenda</label>
                        <div class="col-md-6">
                            <span id="detail_tanggal"></span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="detail_deskripsi" class="control-label col-md-6">Deskripsi Agenda</label>
                        <div class="col-md-6">
                            <span id="detail_deskripsi"></span>
                        </div>
                    </div>
                </div>
            </div><!-- /.modal-content -->
...
@section('js')
    ...
    <script>
        var dataTables = $('#table_agenda').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.agenda.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
                ...
                $.ajax({
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#form_agenda')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#table_agenda').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.agenda.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_tgl').text(data.result.tgl);
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.agenda.edit', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.agenda.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
