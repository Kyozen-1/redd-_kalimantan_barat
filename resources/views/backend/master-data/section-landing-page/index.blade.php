    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_md_section_landing_page" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_md_section_landing_page" method="POST" data-parsley-validate novalidate>
                        @csrf
                        <div class="form-group">
                            <label for="nama" class="control-label">Nama Section Landing Page<span class="text-danger">*</span></label>
                            <input type="text" name="nama" id="nama" parsley-trigger="change" required
                            placeholder="Masukan nama section landing page..." class="form-control">
                        </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="detail_nama" class="control-label col-md-6">Nama Section Landing Page</label>
                        <div class="col-md-6">
...
@section('js')
    ...
    <script>
        var dataTables = $('#table_md_section_landing_page').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.master-data.section-landing-page.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $('#create').click(function(){
            $('#form_md_section_landing_page')[0].reset();
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.section-landing-page.show', ['id' => ":id"]) }}";
        ...
        $('#form_md_section_landing_page').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('cms.master-data.section-landing-page.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#form_md_section_landing_page')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#table_md_section_landing_page').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('cms.master-data.section-landing-page.update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.section-landing-page.edit', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.section-landing-page.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
