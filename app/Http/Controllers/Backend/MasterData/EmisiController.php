...
class EmisiController extends Controller
{
    ...
    {
        return view('backend.master-data.emisi.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $emisi = new MdEmisi;
            $emisi->user_id = Auth::user()->id;
            $emisi->nama = $request->nama;
            $emisi->save();

            return response()->json(['success' => 'Berhasil menambahkan emisi']);
        } catch (\Throwable $th) {
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $emisi = MdEmisi::find($id);
            $emisi->nama = $request->nama;
            $emisi->save();

            return response()->json(['success' => 'Berhasil merubah emisi']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            $id = Crypt::decryptString($id);
            $emisi = MdEmisi::find($id);
            $emisi->status_aktif = '0';
            $emisi->save();

            return response()->json(['success' => 'Berhasil menghapus emisi']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
