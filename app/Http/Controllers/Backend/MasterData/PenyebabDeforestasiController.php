...
class PenyebabDeforestasiController extends Controller
{
    /**
    ...
    {
        return view('backend.master-data.penyebab-deforestasi.index');
    }

    public function datatable()
    {
        $data = new MdPenyebabDeforestasi;
    ...
    public function store(Request $request)
    {
        ...
        try {
            $penyebabDeforestasi = new MdPenyebabDeforestasi;
            $penyebabDeforestasi->user_id = Auth::user()->id;
            $penyebabDeforestasi->nama = $request->nama;
            $penyebabDeforestasi->save();

            return response()->json(['success' => 'Berhasil menambahkan penyebab deforestasi']);
        } catch (\Throwable $th) {
    ...
    public function show(string $id)
    {
        ...
        $getData = MdPenyebabDeforestasi::find($id);
        $dataSend = [
            'nama' => $getData->nama
        ];
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $penyebabDeforestasi = MdPenyebabDeforestasi::find($id);
            $penyebabDeforestasi->nama = $request->nama;
            $penyebabDeforestasi->save();

            return response()->json(['success' => 'Berhasil merubah penyebab deforestasi']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            $id = Crypt::decryptString($id);
            $penyebabDeforestasi = MdPenyebabDeforestasi::find($id);
            $penyebabDeforestasi->status_aktif = '0';
            $penyebabDeforestasi->save();

            return response()->json(['success' => 'Berhasil menghapus penyebab deforestasi']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
