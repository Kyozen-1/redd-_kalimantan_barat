...
@section('title', 'Laporan Emisi | REDD++ Kalimantan Barat')
@section('header', 'Laporan Emisi')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_laporan_emisi" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th width="10%">Aksi</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_laporan_emisi" method="POST" enctype="multipart/form-data" data-parsley-validate novalidate>
                        @csrf
...
@section('js')
    ...
    <script>
        $('#kategori_id').select2();
        $('.dropify').dropify();
        var dataTables = $('#table_laporan_emisi').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.laporan-emisi.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $('#form_laporan_emisi').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('cms.laporan-emisi.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            reset();
                            $('#aksi_button').text('Save');
                            $('#table_laporan_emisi').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('cms.laporan-emisi.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.laporan-emisi.edit', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.laporan-emisi.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
