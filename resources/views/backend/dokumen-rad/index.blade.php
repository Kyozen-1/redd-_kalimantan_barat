...
@section('title', 'Dokumen RAD | REDD++ Kalimantan Barat')
@section('header', 'Dokumen RAD')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_dokumen_rad" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th width="10%">Aksi</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_dokumen_rad" method="POST" enctype="multipart/form-data" data-parsley-validate novalidate>
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="document" class="control-label">Dokumen PDF <small class="text-danger">*diisi jika ada</small></label>
                            <input type="file" class="dropify" id="document" name="document" data-height="150" data-allowed-file-extensions="pdf" data-show-errors="true">
                        </div>
                </div>
...
@section('js')
    ...
    <script>
        $('.dropify').dropify();
        var dataTables = $('#table_dokumen_rad').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.dokumen-rad.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $('#form_dokumen_rad').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('cms.dokumen-rad.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            reset();
                            $('#aksi_button').text('Save');
                            $('#table_dokumen_rad').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('cms.dokumen-rad.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.dokumen-rad.edit', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    var lokasiDocument = data.result.document;
                    var fileDropperLokasiDocument = $("#document").dropify();

                    fileDropperLokasiDocument = fileDropperLokasiDocument.data('dropify');
                    fileDropperLokasiDocument.resetPreview();
                    fileDropperLokasiDocument.clearElement();
                    fileDropperLokasiDocument.settings['defaultFile'] = lokasiDocument;
                    fileDropperLokasiDocument.destroy();
                    fileDropperLokasiDocument.init();
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.dokumen-rad.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
