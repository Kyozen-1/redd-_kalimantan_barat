...
class KawasanHutanController extends Controller
{
    ...
    {
        return view('backend.master-data.kawasan-hutan.index');
    }

    public function datatable()
    {
        $data = new MdKawasanHutan;
    ...
    public function store(Request $request)
    {
        ...
        try {
            $kawasanHutan = new MdKawasanHutan;
            $kawasanHutan->user_id = Auth::user()->id;
            $kawasanHutan->nama = $request->nama;
            $kawasanHutan->save();

            return response()->json(['success' => 'Berhasil menambahkan kawasan hutan']);
        } catch (\Throwable $th) {
    ...
    public function show(string $id)
    {
        ...
        $getData = MdKawasanHutan::find($id);
        $dataSend = [
            'nama' => $getData->nama
        ];
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $kawasanHutan = MdKawasanHutan::find($id);
            $kawasanHutan->nama = $request->nama;
            $kawasanHutan->save();

            return response()->json(['success' => 'Berhasil merubah kawasan hutan']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            $id = Crypt::decryptString($id);
            $kawasanHutan = MdKawasanHutan::find($id);
            $kawasanHutan->status_aktif = '0';
            $kawasanHutan->save();

            return response()->json(['success' => 'Berhasil menghapus kawasan hutan']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
