...
class SektorEmisiController extends Controller
{
    /**
    ...
    {
        return view('backend.master-data.sektor-emisi.index');
    }

    public function datatable()
    {
        $data = new MdSektorEmisi;
    ...
    public function store(Request $request)
    {
        ...
        try {
            $sektorEmisi = new MdSektorEmisi;
            $sektorEmisi->user_id = Auth::user()->id;
            $sektorEmisi->nama = $request->nama;
            $sektorEmisi->save();

            return response()->json(['success' => 'Berhasil menambahkan sektor emisi']);
        } catch (\Throwable $th) {
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $sektorEmisi = MdSektorEmisi::find($id);
            $sektorEmisi->nama = $request->nama;
            $sektorEmisi->save();

            return response()->json(['success' => 'Berhasil merubah sektor emisi']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            $id = Crypt::decryptString($id);
            $sektorEmisi = MdSektorEmisi::find($id);
            $sektorEmisi->status_aktif = '0';
            $sektorEmisi->save();

            return response()->json(['success' => 'Berhasil menghapus sektor emisi']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
