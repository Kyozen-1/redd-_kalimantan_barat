...
@section('title', 'Sektor Emisi | Master Data | REDD++ Kalimantan Barat')
@section('header', 'Sektor Emisi | Master Data')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_md_sektor_emisi" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_md_sektor_emisi" method="POST" data-parsley-validate novalidate>
                        @csrf
                        <div class="form-group">
                            <label for="nama" class="control-label">Nama Sektor Emisi<span class="text-danger">*</span></label>
                            <input type="text" name="nama" id="nama" parsley-trigger="change" required
                            placeholder="Masukan nama Sektor Emisi..." class="form-control">
                        </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="detail_nama" class="control-label col-md-6">Nama Sektor Emisi</label>
                        <div class="col-md-6">
...
@section('js')
    ...
    <script>
        var dataTables = $('#table_md_sektor_emisi').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.master-data.sektor-emisi.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.sektor-emisi.show', ['id' => ":id"]) }}";
        ...
        $('#form_md_sektor_emisi').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('cms.master-data.sektor-emisi.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#form_md_sektor_emisi')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#table_md_sektor_emisi').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('cms.master-data.sektor-emisi.update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.sektor-emisi.edit', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.sektor-emisi.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
